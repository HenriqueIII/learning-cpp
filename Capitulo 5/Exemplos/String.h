...
class String{
    char * str;
    int cp; //Dimensão do espaço reservado para a cadeia
...
public:
    ...
    //operador de afectação
    String & operator= (const String &s){assignStr(s);return *this; }
};

//Constroi String com string de dimensão lenght
...
void String::assignStr(const char * s, int lenght){
    if (lenght == -1)
        lenght = (s)?strlen(s):0;
    if (lenght + 1 > cp){               //Se espaço insuficiente
        delete [] str;                  //Devolve espaço anterior
        str = new char[cp=lenght+1];    //Reserva novo Espaço
    }
    if(lenght)
...
void String::insertChar(int idx, char c){
    ...
    if (len + 1 == cp){
        ...
        delete [] aux;          //Disponibiliza o espaço anterior
    }
    for (int i=len;i>=idx; --i) //Abre Espaço
        str[i+1] = str[i];
    str[idx] = c;
    ++len;
}
